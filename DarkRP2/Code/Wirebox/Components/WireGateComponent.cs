...
public partial class WireGateComponent : BaseWireInputOutputComponent, Component.IPressable
{
	...
	public override void WireInitialize()
	{
		...
		if ( renderer.IsValid() && renderer.Model is not null && renderer.Model.Name.Contains( "chip_rectangle.vmdl" ) )
		{
			...
			if ( renderer.Model.MaterialGroupCount > index )
				renderer.MaterialGroup = renderer.Model.GetMaterialGroupName( index );
		}
		...
		else if ( GateType == "Owner" )
		{
			...
			var ownerPlayer = owner is not null ? Player.FindForConnection( owner ) : null;
			if ( ownerPlayer.IsValid() )
				this.WireTriggerOutput( "Out", ownerPlayer.GameObject );
		}
	...
	protected override void OnFixedUpdate()
	{
		...
		{
			this.WireTriggerOutput( "Out", storedEnt.IsValid() ? storedEnt.GetComponent<Rigidbody>()?.Velocity ?? Vector3.Zero : Vector3.Zero );
		}
	}
